...
HELIX_TURNS = 7.8                 # Number of spiral turns
HELIX_HEIGHT = PIXEL_COUNT     # LED count = total "height"
SPARK_COUNT = 8                # How many new sparks per frame
SPIRAL_DRIFT = 0.196             # Radians of spiral twist per LED
...
pixels_np = np.array(pixels, dtype=np.int16)  # numpy working copy of pixel data
fire_fade_by = np.array( (-3,-3,-3), dtype=np.int16 )  # amount to fade by
...
def fire():
    ...
    for _ in range(SPARK_COUNT):
        r = random.random()
        index = PIXEL_COUNT - 1 - int((1.0 - r**2) * (PIXEL_COUNT // 8))  # biased toward bottom
    ...
    pixels_np += fire_fade_by

    # Cooling and spiral twist—adjusted for top-down indexing
    for i in range(PIXEL_COUNT):
        # Translate top-down index to bottom-up for cooling math
        inv_i = PIXEL_COUNT - 1 - i

        # Cooling gradient: cools from bottom (high index) to top (low index)
        vertical_cool = 1.0 - (inv_i / PIXEL_COUNT) * 0.5  # 1.0 at bottom, 0.5 at top
